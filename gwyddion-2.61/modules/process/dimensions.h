/*
 *  $Id: dimensions.h 23211 2021-03-11 11:14:21Z yeti-dn $
...
typedef struct {
    ...
    GwySIValueFormat *sxyvf;  /* Small xy: for objects typically << image. */
                             ...
                             gchar **unitstr)
{
    ...
    gwy_assign_string(unitstr, unit);
                               ...
                               GtkComboBox *combo)
{
    ...
    args->xypow10 = gwy_enum_combo_box_get_active(combo);
    gwy_si_unit_set_from_string(dims->xysiunit, args->xyunits);
    gwy_si_unit_get_format_for_power10(dims->xysiunit,
                                       GWY_SI_UNIT_FORMAT_VFMARKUP,
                                       args->xypow10, dims->xyvf);
    res = MAX(args->xres, args->yres);
    size = res*args->measure * pow10(args->xypow10);
    gwy_si_unit_get_format_with_resolution(dims->xysiunit,
                                           GWY_SI_UNIT_FORMAT_VFMARKUP,
                                           size, size/res, dims->sxyvf);
...
gwy_dimensions_use_template(GwyDimensions *dims)
{
    ...
    xyvf = gwy_data_field_get_value_format_xy(dims->template_,
                                              GWY_SI_UNIT_FORMAT_PLAIN, NULL);
    gwy_data_field_get_min_max(dims->template_, &min, &max);
    /* The height control has a high precision.  We are more likely to run into
     * trouble when the template is relatively flat. */
    if (max == min) {
        max = ABS(max);
        min = 0.0;
    }
    m = 12.0*(max - min);
    zunit = gwy_data_field_get_si_unit_z(dims->template_);
    zvf = gwy_si_unit_get_format_with_digits(zunit, GWY_SI_UNIT_FORMAT_PLAIN,
                                             m, 3, NULL);
    ...
    gwy_assign_string(&dims->args->xyunits, xyvf->units);
    gwy_dimensions_set_combo_from_unit(GTK_COMBO_BOX(dims->xypow10),
                                       xyvf->units);
    gwy_assign_string(&dims->args->zunits, zvf->units);
                   ...
                   GwyDataField *template_)
{
    ...
    res = MAX(args->xres, args->yres);
    size = res*args->measure * pow10(args->xypow10);
    dims->sxyvf = gwy_si_unit_get_format_with_resolution(dims->xysiunit,
                                                         GWY_SI_UNIT_FORMAT_VFMARKUP,
                                                         size, size/res, NULL);
...
gwy_dimensions_free(GwyDimensions *dims)
{
    ...
    gwy_si_unit_value_format_free(dims->sxyvf);
                         ...
                         const gchar *prefix)
{
    ...
    if (gwy_container_gis_string_by_name(settings, key->str, &s)) {
        gwy_assign_string(&args->xyunits, s);
    }

    g_string_append(g_string_truncate(key, len), "zunits");
    if (gwy_container_gis_string_by_name(settings, key->str, &s)) {
        gwy_assign_string(&args->zunits, s);
    }
