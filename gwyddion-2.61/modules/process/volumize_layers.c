/*
 *  $Id: volumize_layers.c 23211 2021-03-11 11:14:21Z yeti-dn $
...
volumize_layers(GwyContainer *data, GwyRunType run)
{
    ...
    gwy_si_unit_assign(gwy_brick_get_si_unit_y(brick),
                       gwy_data_field_get_si_unit_xy(dfield));
    gwy_si_unit_assign(gwy_brick_get_si_unit_w(brick),
                       ...
                       gint xres, gint yres, gint zres)
{
    ...
    controls.xres = gtk_adjustment_new(args->xres, 1.0, 16384.0, 1, 10, 0);
    gwy_table_attach_adjbar(table, row, _("_X resolution:"), _("px"),
                            controls.xres, GWY_HSCALE_SQRT);
    row++;

    controls.yres = gtk_adjustment_new(args->yres, 1.0, 16384.0, 1, 10, 0);
...
change_zunits(VolumizeLayersControls *controls)
{
    ...
    gwy_assign_string(&controls->args->zunit,
                          ...
                          VolumizeLayersArgs *args)
{
    ...
    *args = volumize_layers_defaults;
    ...
    if (gwy_container_gis_string_by_name(container, zunit_key, &s)) {
        gwy_assign_string(&args->zunit, s);
    }
}
