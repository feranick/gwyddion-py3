/*
 *  $Id: graph_fit.c 24708 2022-03-21 17:14:46Z yeti-dn $
...
fit_dialog(FitArgs *args, GwyContainer *container)
{
    ...
    gtk_widget_set_tooltip_text(button, _("Copy all fitted values to estimates"));
...
fit_do(FitControls *controls)
{
    ...
    args->fitter
        = gwy_nlfit_preset_fit(args->fitfunc, NULL,
                               ...
                               param, error, fixed);
    errorknown = (gwy_math_nlfit_get_covar(args->fitter) != NULL);
                           ...
                           gboolean errorknown)
{
    ...
    if (!gwy_math_nlfit_get_eval(controls->args->fitter)) {
        gtk_label_set_text(GTK_LABEL(cntrl->value), "");
        gtk_label_set_text(GTK_LABEL(cntrl->value_unit), "");
        gtk_label_set_text(GTK_LABEL(cntrl->error), "");
        gtk_label_set_text(GTK_LABEL(cntrl->error_unit), "");
...
create_fit_report(FitArgs *args)
{
    ...
    g_return_val_if_fail(gwy_math_nlfit_get_covar(args->fitter), report);
