/*
 *  $Id: volume_mfmrecalc.c 21471 2018-09-26 19:47:48Z klapetek $
...
#define VMFM_RECALC_RUN_MODES (GWY_RUN_IMMEDIATE | GWY_RUN_INTERACTIVE)
...
static void     volume_mfmrecalc               (GwyContainer *data,
                                                GwyRunType run);
static gboolean vmfm_recalc_dialog             (VMfmRecalcArgs *args,
                                                MfmRecalcSignal guess);
static void     update_sensitivity             (VMfmRecalcControls *controls);
static void     vmfm_recalc_sanitize_args      (VMfmRecalcArgs *args);
static void     vmfm_recalc_load_args          (GwyContainer *container,
                                                VMfmRecalcArgs *args);
static void     vmfm_recalc_save_args          (GwyContainer *container,
                                                VMfmRecalcArgs *args);
static void     signal_changed                 (GtkToggleButton *toggle,
                                                VMfmRecalcControls *controls);
static void     new_channel_changed            (GtkToggleButton *check,
                                                VMfmRecalcControls *controls);
static void     vmfm_recalc_dialog_update      (VMfmRecalcControls *controls);
static void     mfm_recalc_dialog_update_values(VMfmRecalcControls *controls,
                                                VMfmRecalcArgs *args);

static const VMfmRecalcArgs vmfm_recalc_defaults = {
...
module_register(void)
{
    gwy_volume_func_register("volume_mfmrecalc",
                             ...
                             N_("/_Magnetic Data to Force Gradient..."),
                             NULL,
                             VMFM_RECALC_RUN_MODES,
...
{
    VMfmRecalcArgs args;
    GwyBrick *brick = NULL, *out;
    GwyDataField *preview;
    gint oldid, newid, xres, yres;
    gboolean ok;
    GwySIUnit *zunit;
    MfmRecalcSignal guess;
    GQuark bquark, pquark;

    g_return_if_fail(run & VMFM_RECALC_RUN_MODES);
    vmfm_recalc_load_args(gwy_app_settings_get(), &args);
    gwy_app_data_browser_get_current(GWY_APP_BRICK, &brick,
                                     GWY_APP_BRICK_ID, &oldid,
                                     GWY_APP_BRICK_KEY, &bquark,
                                     0);
    g_return_if_fail(GWY_IS_BRICK(brick));
    pquark = gwy_app_get_brick_preview_key_for_id(oldid);

    //guess the units
    zunit = gwy_brick_get_si_unit_w(brick);
    ...
    if (run == GWY_RUN_INTERACTIVE) {
        ok = vmfm_recalc_dialog(&args, guess);
        vmfm_recalc_save_args(gwy_app_settings_get(), &args);
        if (!ok)
            return;
    }

    if (args.new_channel)
        out = gwy_brick_duplicate(brick);
    else {
        GQuark quarks[2];
    ...
    if (args.signal == SIGNAL_PHASE_DEG) {
        gwy_brick_mfm_phase_to_force_gradient(out,
                                              args.spring_constant,
                                              args.quality,
                                              args.result);
        gwy_brick_multiply(out, M_PI/180);
    }
        ...
        gwy_brick_mfm_frequency_shift_to_force_gradient(out,
                                                        args.spring_constant,
                                                        args.base_frequency,
                                                        args.result);
    }
    else if (args.signal == SIGNAL_AMPLITUDE_M) {
        gwy_brick_mfm_amplitude_shift_to_force_gradient(out,
                                                        args.spring_constant,
                                                        args.quality,
    ...
    xres = gwy_brick_get_xres(out);
    yres = gwy_brick_get_yres(out);
    preview = gwy_data_field_new(xres, yres, xres, yres, FALSE);
    gwy_brick_mean_xy_plane(out, preview);

    if (args.new_channel) {
        newid = gwy_app_data_browser_add_brick(out, preview, data, TRUE);
        gwy_app_set_brick_title(data, oldid, _("Recalculated Data"));
        gwy_app_volume_log_add_volume(data, oldid, newid);
        gwy_app_sync_volume_items(data, data, oldid, newid,
                                  GWY_DATA_ITEM_GRADIENT,
                                  0);
        g_object_unref(out);
    }
    else {
        gwy_brick_data_changed(out);
        gwy_app_volume_log_add_volume(data, oldid, oldid);
        gwy_container_set_object(data, pquark, preview);
    }
    g_object_unref(preview);
}

static gboolean
vmfm_recalc_dialog(VMfmRecalcArgs *args, MfmRecalcSignal guess)
{
    ...
    dialog = gtk_dialog_new_with_buttons(_("MFM Recalculate Data"),
                                         NULL, 0,
    ...
    do {
        ...
        switch (response) {
            ...
            case RESPONSE_RESET:
            *args = vmfm_recalc_defaults;
            args->signal = guess;
            vmfm_recalc_dialog_update(&controls);
            break;
...
static void
update_sensitivity(VMfmRecalcControls *controls)
{
    VMfmRecalcArgs *args = controls->args;
    GtkWidget *label;
...
static void
vmfm_recalc_dialog_update(VMfmRecalcControls *controls)
{
    VMfmRecalcArgs *args = controls->args;
...
static void
mfm_recalc_dialog_update_values(VMfmRecalcControls *controls,
                                VMfmRecalcArgs *args)
{
    args->spring_constant
...
static void
signal_changed(GtkToggleButton *toggle, VMfmRecalcControls *controls)
{
...
static const gchar base_amplitude_key[]  = "/module/volume_mfmrecalc/base_amplitude";
static const gchar base_frequency_key[]  = "/module/volume_mfmrecalc/base_frequency";
static const gchar new_channel_key[]     = "/module/volume_mfmrecalc/new_channel";
static const gchar quality_key[]         = "/module/volume_mfmrecalc/quality";
static const gchar signal_key[]          = "/module/volume_mfmrecalc/signal";
static const gchar spring_constant_key[] = "/module/volume_mfmrecalc/spring_constant";
static const gchar result_key[]          = "/module/volume_mfmrecalc/result";


static void
vmfm_recalc_sanitize_args(VMfmRecalcArgs *args)
{
...
static void
vmfm_recalc_load_args(GwyContainer *container,
              VMfmRecalcArgs *args)
{
    *args = vmfm_recalc_defaults;
    ...
    vmfm_recalc_sanitize_args(args);
}

static void
vmfm_recalc_save_args(GwyContainer *container,
              VMfmRecalcArgs *args)
{
