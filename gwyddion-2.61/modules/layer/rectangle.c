/*
 *  $Id: rectangle.c 24732 2022-03-23 20:01:21Z yeti-dn $
 ...
 */
/* FIXME: crop() can really crop the recangles */
#include "config.h"
...
static gboolean gwy_selection_rectangle_crop_object(GwySelection *selection,
                                                    gint i,
                                                    gpointer user_data);
static void     gwy_selection_rectangle_crop       (GwySelection *selection,
                                                    gdouble xmin,
                                                    gdouble ymin,
                                                    gdouble xmax,
                                                    gdouble ymax);
...
static gboolean gwy_layer_rectangle_button_released(GwyVectorLayer *layer,
                                                    GdkEventButton *event);
static gboolean gwy_layer_rectangle_key_pressed    (GwyVectorLayer *layer,
                                                    GdkEventKey *event);
...
static gboolean
gwy_selection_rectangle_crop_object(GwySelection *selection,
                                    gint i,
                                    gpointer user_data)
{
...
static void
gwy_selection_rectangle_crop(GwySelection *selection,
                             gdouble xmin,
                             gdouble ymin,
                             gdouble xmax,
                             gdouble ymax)
...
gwy_layer_rectangle_class_init(GwyLayerRectangleClass *klass)
{
    ...
    vector_class->key_press = gwy_layer_rectangle_key_pressed;
                                ...
                                gint i)
{
    ...
    gwy_layer_rectangle_draw_rectangle(layer, drawable, target, xy);
...
static gboolean
gwy_layer_rectangle_key_pressed(GwyVectorLayer *layer,
                                GdkEventKey *event)
{
    ...
    gwy_data_view_coords_real_to_xy(data_view, xy[which], xy[which+1],
                                    &xcurr, &ycurr);
    xnew = xcurr;
    ynew = ycurr;
    ...
    if (xnew != xcurr || ynew != ycurr) {
        gwy_data_view_coords_xy_to_real(data_view, xnew, ynew,
                                        xy+which, xy+which+1);
                               ...
                               gdouble xreal, gdouble yreal)
{
    ...
    if (layer->focus >= 0) {
        ...
        gdouble *coords = g_new(gdouble, 8*n);
