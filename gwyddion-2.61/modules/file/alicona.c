/*
 *  $Id: alicona.c 23594 2021-04-28 20:02:16Z yeti-dn $
          ...
          GError **error)
{
    ...
    /* XXX: Some files apparently do not give NumberOfPlanes but they have
     * 3 planes.  Try to guess the number from TexturePtr? */
                 ...
                 GwyDataField **badmask)
{
    ...
    /* If InvalidPixelValue is finite it does not have full double precision.
     * After all, the raw data are single precision.  Use a loose comparison to
    ...
    for (i = 0; i < yres; i++) {
        ...
        for (j = 0; j < xres; j++, d++, m++) {
            *d = gwy_get_gfloat_le(&p);
            if (gwy_isnan(*d)
                || (!invalid_is_nan
                    && fabs(*d - invalid_value) < invalid_eps)) {
                if (!mask) {
