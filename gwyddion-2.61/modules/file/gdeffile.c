/*
 *  $Id: gdeffile.c 22723 2020-01-30 15:54:47Z yeti-dn $
...
typedef struct {
    ...
    guint version;
    guchar alignment[2];
    /* On-disk this is ‘32bit time_t’.  We assume it can be safely assigned to
     * a larger integer when actual time_t is larger. */
                        ...
                        GError **error)
{
    ...
    *p += 2;
    ...
    gwy_debug("n_variables: %u, n_data: %u, next: %u",
              block->n_variables, block->n_data, !!block->next);
    *p += 3;
        ...
        for (i = 0; i < block->n_variables; i++) {
            ...
            if (var->type == VAR_DATABLOCK) {
                ...
                *p += var->size;
                if (*p > buffer + size) {
                    if (error) {
                        gchar *s = g_strconcat("GDEF Variable ", var->name,
                                               NULL);
    ...
    for (block = blocks; block; block = block->next) {
        ...
        for (j = 0; j < block->n_variables; j++) {
            ...
            if (var->type != VAR_DATABLOCK)
                gwy_debug("%s 0x%08x",
                          var->name,
                          (guint)((const guchar*)var->data - buffer));
        }
#endif
        ...
        if ((dfield = gdef_read_channel(block))) {
            ...
            var = gdef_find_variable(block, "SourceChannel", VAR_INTEGRAL,
                                     FALSE);
            if (var) {
