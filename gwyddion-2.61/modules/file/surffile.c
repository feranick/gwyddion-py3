/*
 *  $Id: surffile.c 24038 2021-08-21 11:18:28Z yeti-dn $
...
/* New structure surfwriter to write directly the structure with cast gint16 of
 * enums */
typedef struct {
    char signature[12];
    gint16 format;
...
static gboolean      surffile_save        (GwyContainer *data,
                                           ...
                                           GError **error);
static void          format_unit          (char *unit,
                                           int *power,
                                           double ref);
...
{
    get_CHARS0(ustr, p, 16);
    /* Fix µ symbol from old MS-DOS code pages to Latin1 code.  Apparently that
              ...
              GError **error)
{
    ...
    gwy_data_field_multiply(surffile.dfield, pow10(coef)*surffile.dz);
                 ...
                 GError **error)
{
    ...
    surffile->dfield = field = gwy_data_field_new(surffile->xres,
                                                  surffile->yres,
    ...
    gwy_convert_raw_data(buffer, n, 1, datatype, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                         data, 1.0, 0.0);

    gwy_data_field_set_si_unit_xy(field, surffile->xyunit);
    g_object_unref(surffile->xyunit);

    gwy_data_field_set_si_unit_z(field, surffile->zunit);
...
static void
format_unit(char *unit, gint *power, gdouble ref)
{
    static const gchar *units[5] = { "pm", "nm", "\xb5m", "mm", "m" };
    int i = 0;

    ref = fabs(ref); //return no error if ref <= 0
    while ((log(ref)/G_LN10 < -11 + (i*3)) && (++i < 4))
        ;
