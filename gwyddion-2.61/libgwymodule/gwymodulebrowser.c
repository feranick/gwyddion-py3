/*
 *  $Id: gwymodulebrowser.c 24033 2021-08-20 14:28:15Z yeti-dn $
...
gwy_module_browser(void)
{
    ...
    g_signal_connect_swapped(window, "destroy", G_CALLBACK(g_nullify_pointer), &window);
...
gwy_module_browser_construct(GtkWidget *parent)
{
    ...
    store = gtk_list_store_new(MODEL_LAST, G_TYPE_POINTER, G_TYPE_POINTER, G_TYPE_BOOLEAN);
    ...
    gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(store), MODEL_NAME, compare_modules, NULL, NULL);
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(store), MODEL_NAME, GTK_SORT_ASCENDING);
    ...
    for (i = 0; i < G_N_ELEMENTS(columns); i++) {
        ...
        column = gtk_tree_view_column_new_with_attributes(_(columns[i]), renderer, NULL);
        gtk_tree_view_column_set_cell_data_func(column, renderer, gwy_module_browser_render, GUINT_TO_POINTER(i), NULL);
    ...
    g_signal_connect(tree, "cursor-changed", G_CALLBACK(update_module_info_cb), parent);
                      ...
                      GtkWidget *parent)
{
    ...
    if (failed) {
        ...
        gtk_label_set_text(GTK_LABEL(g_object_get_data(G_OBJECT(parent), "fail-name")), name);
        gtk_label_set_text(GTK_LABEL(g_object_get_data(G_OBJECT(parent), "fail-file")), fail_info->filename);
        gtk_label_set_text(GTK_LABEL(g_object_get_data(G_OBJECT(parent), "failure")), fail_info->err_message);
        return;
    }
    ...
    gtk_label_set_text(GTK_LABEL(g_object_get_data(G_OBJECT(parent), "name-version")), s);
    g_free(s);

    gtk_label_set_text(GTK_LABEL(g_object_get_data(G_OBJECT(parent), "file")), gwy_module_get_filename(name));
    gtk_label_set_text(GTK_LABEL(g_object_get_data(G_OBJECT(parent), "author")), mod_info->author);
    gtk_label_set_text(GTK_LABEL(g_object_get_data(G_OBJECT(parent), "copy")), mod_info->copyright);
    gtk_label_set_text(GTK_LABEL(g_object_get_data(G_OBJECT(parent), "date")), mod_info->date);
    gtk_label_set_text(GTK_LABEL(g_object_get_data(G_OBJECT(parent), "desc")), _(mod_info->blurb));
    ...
    gtk_label_set_text(GTK_LABEL(g_object_get_data(G_OBJECT(parent), "funcs")), str->str);
                 ...
                 const gchar *key)
{
    ...
    multiline = gwy_stramong(key, "desc", "funcs", "failure", NULL);
