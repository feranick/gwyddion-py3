/*
 *  $Id: settings.c 24546 2021-11-26 10:44:01Z yeti-dn $
                      ...
                      GError **error)
{
    ...
    if (!size || !buffer) {
        g_set_error(error, GWY_APP_SETTINGS_ERROR, GWY_APP_SETTINGS_ERROR_EMPTY,
                    _("File is empty."));
        g_free(buffer);
        return FALSE;
    }
...
gwy_app_settings_set_defaults(GwyContainer *settings)
{
    ...
    if (!gwy_container_contains_by_name(settings, "/app/gradients/preferred"))
        gwy_container_set_string_by_name(settings, "/app/gradients/preferred", g_strdup(default_preferred_gradients));
