/*
 *  $Id: gwydatawindow.c 24710 2022-03-21 17:35:20Z yeti-dn $
...
gwy_data_window_init(GwyDataWindow *data_window)
{
    ...
    data_window->str = g_string_new(NULL);
}
...
gwy_data_window_finalize(GObject *object)
{
    ...
    g_string_free(data_window->str, TRUE);
    g_free(data_window->value_unit);
...
gwy_data_window_new(GwyDataView *data_view)
{
    ...
    */
    ...
    gwy_data_window_fit_to_screen(data_window);
 ...
 * @data_window: A data view window.
 *
 * Sets the zoom of data windows's data view in an attempt to make the window
 * fit to the screen.
 *
 * Since: 2.56
 **/
...
gwy_data_window_fit_to_screen(GwyDataWindow *data_window)
{
    ...
    zoom = gwy_data_view_get_zoom(GWY_DATA_VIEW(data_window->data_view));
    gtk_widget_size_request(data_window->data_view, &request);
    z = MAX(request.width/(gdouble)scrwidth, request.height/(gdouble)scrheight);
    if (z > 0.9) {
        zoom *= 0.9/z;
        gwy_data_view_set_zoom(GWY_DATA_VIEW(data_window->data_view), zoom);
        GTK_WIDGET_CLASS(gwy_data_window_parent_class)->size_request(widget,
                                                                     &request);
        if (request.width && request.height) {
            gtk_window_resize(GTK_WINDOW(data_window),
...
gwy_data_window_update_units(GwyDataWindow *data_window)
{
    ...
    data_window->value_format
        = gwy_data_field_get_value_format_z(dfield,
                                            ...
                                            data_window->value_format);
    g_free(data_window->value_unit);
    data_window->value_unit = gwy_si_unit_get_string(zunit,
                                                     GWY_SI_UNIT_FORMAT_VFMARKUP);
    ...
    gwy_ruler_set_si_unit(GWY_RULER(data_window->hruler), xyunit);
    gwy_ruler_set_si_unit(GWY_RULER(data_window->vruler), xyunit);
    gwy_color_axis_set_si_unit(GWY_COLOR_AXIS(data_window->coloraxis), zunit);
}
                                 ...
                                 GwyDataWindow *data_window)
{
    ...
    GwySIValueFormat *xyvf, *zvf;
    gint x, y;

    if (!data_view) {
        gwy_statusbar_set_markup(GWY_STATUSBAR(data_window->statusbar), "");
        return FALSE;
    }
    ...
    xyvf = data_window->coord_format;
    zvf = data_window->value_format;
    g_string_printf(data_window->str, "(%.*f%s%s, %.*f%s%s): %.*f%s%s",
    ...
    if (fabs(log(zvf->magnitude)) > 1e-12) {
        g_string_append_printf(data_window->str, " = %.3e %s",
                               value, data_window->value_unit);
    }
    gwy_statusbar_set_markup(GWY_STATUSBAR(data_window->statusbar),
                             data_window->str->str);
...
gwy_data_window_update_title(GwyDataWindow *data_window)
{
    ...
    if (zoom >= 1.0) {
        ...
        g_string_printf(data_window->str,
                        "%s %.*f:1 (%s)",
    ...
    gtk_window_set_title(GTK_WINDOW(data_window), data_window->str->str);
}
