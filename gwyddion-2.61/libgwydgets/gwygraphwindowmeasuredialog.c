/*
 *  $Id: gwygraphwindowmeasuredialog.c 24180 2021-09-23 08:21:24Z yeti-dn $
...
static void     method_cb                                           (GtkWidget *combo,
                                                                     GwyGraphWindowMeasureDialog *dialog);
static void     n_curves_changed_cb                                 (GwyGraphWindowMeasureDialog *dialog);
                     ...
                     GwyGraphWindowMeasureDialog *dialog)
{
    ...
    if (gwy_graph_model_get_n_curves(gmodel) == 0) {
        //printf("no curves! This will segfault.\n");
        n = 0;
    }
    else {
...
static void
n_curves_changed_cb(GwyGraphWindowMeasureDialog *dialog)
{
    GwyGraphModel *gmodel;
    GwyGraphArea *garea;
    GwyGraphStatusType status;
    ...
    if (n == 0) {
    //    printf("what shell we do with 0 curves?");
        garea = GWY_GRAPH_AREA(gwy_graph_get_area(GWY_GRAPH(dialog->graph)));
        status = gwy_graph_area_get_status(garea);
        selection = gwy_graph_area_get_selection(garea, status);
        if (selection)
            gwy_selection_clear(selection);
    }


    oldn = gtk_adjustment_get_value(GTK_ADJUSTMENT(dialog->index));
    if (oldn > n)
        gtk_adjustment_set_value(GTK_ADJUSTMENT(dialog->index), n);

    gtk_adjustment_set_upper(GTK_ADJUSTMENT(dialog->index), gwy_graph_model_get_n_curves(gmodel));
    index_changed_cb(dialog);
...
_gwy_graph_window_measure_dialog_new(GwyGraph *graph)
{
    ...
    g_signal_connect_swapped(gmodel, "notify::n-curves",
                  G_CALLBACK(n_curves_changed_cb),
                  dialog);
