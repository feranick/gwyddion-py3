/*
 *  $Id: hough.c 24660 2022-03-10 16:16:44Z yeti-dn $
                        ...
                        gint j, gint i, gdouble value, gdouble skip2, gint *count, gint ndata)
{
    ...
    for (k = n = 0; n < *count; n++) {
        ...
        if (dx*dx + dy*dy < skip2) {
            /* If we find a maximum which is too close and larger, then the new one can be discarded. */
            if (zdata[n] >= value)
                return -1;
            /* Replace the first maximum which is too close if it is smaller. */
            if (m == -1) {
                xdata[k] = j;
                ydata[k] = i;
                zdata[k] = value;
            ...
            /* Otherwise item n gets skipped (overwritten). */
        }
        else {
            xdata[k] = xdata[n];
            ydata[k] = ydata[n];
    ...
    /* If the new maximum is not too close to anything find the insertion position normally.  Search from the end;
     * that is where we are likely to find the right position.*/
    if (m == -1) {
        for (k = *count; k && zdata[k-1] < value; k--)
            ;
        g_assert(k < ndata);
        *count = MAX(*count, k+1);
        n = (*count-1 - k);
        memmove(xdata+k+1, xdata+k, n*sizeof(gdouble));
        memmove(ydata+k+1, ydata+k, n*sizeof(gdouble));
        memmove(zdata+k+1, zdata+k, n*sizeof(gdouble));
        xdata[k] = j;
        ydata[k] = i;
        zdata[k] = value;
        return k;
    ...
    /* We replaced something and it could have been more than one item. */
    *count = k;

    /* Ensure the arrays are still sorted.  Removal does not break the order, but item m is larger than the previous
     * item at that position and may have to be moved up. */
    k = m;
    while (m > 0 && zdata[m-1] < zdata[m]) {
        GWY_SWAP(gdouble, xdata[m], xdata[m-1]);
        GWY_SWAP(gdouble, ydata[m], ydata[m-1]);
        GWY_SWAP(gdouble, zdata[m], zdata[m-1]);
        m--;
    }
