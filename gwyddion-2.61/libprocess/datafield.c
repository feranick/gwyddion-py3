/*
 *  $Id: datafield.c 23264 2021-03-15 14:46:40Z yeti-dn $
...
data_field_is_constant(GwyDataField *data_field, gdouble *z)
{
     ...
     * or goes to the end of the field, but then the caller should save a lot
 ...
 *
 * The returned data field has always square pixels.  If you want to rotate
 * by a multiple of %G_PI/2 while preserving non-square pixels, you must use
 * explicitly a function such as gwy_data_field_new_rotated_90().
 *
 * Returns: A newly created data field.
 *
 * Since: 2.46
 ...
 *
 * Note that the axis parameter convention is confusing and different from
 * gwy_brick_invert() and gwy_data_line_invert().  Parameters @x an @y
 * correspond the axes around which to flip (which themselves stay unchanged).
 * You may need to swap @x and @y arguments compared what you would pass
 * naturally.
 **/
void
...
gwy_data_field_multiply(GwyDataField *data_field, gdouble value)
{
    ...
    /* We can transform stats */
    data_field->cached &= CBIT(MIN) | CBIT(MAX) | CBIT(SUM) | CBIT(RMS)
                          | CBIT(MED) | CBIT(ARF) | CBIT(ART) | CBIT(MSQ);
    CVAL(data_field, MIN) *= value;
    CVAL(data_field, MAX) *= value;
...
gwy_data_field_add(GwyDataField *data_field, gdouble value)
{
    ...
    CVAL(data_field, ART) += value;
    /* Area doesn't change */
    /* There is transformation formula for MSQ, but it can be prone to ugly
     * cancellation errors. */
}
...
gwy_data_field_abs(GwyDataField *data_field)
{
    ...
    /* The only cached stat we could transform is the maximum.  That's probably
     * not worth the fuss. */
    gwy_data_field_invalidate(data_field);
}
 ...
 *
 * The data field is divided into rectangles of dimensions @binw×@binh, offset
 * by (@xoff, @yoff).  The values in each complete rectangle are averaged and
 * the average becomes the pixel value in the newly created, smaller data
 * field.
 *
 * Note that the result is the average – not sum – of the individual values.
 * Multiply the returned data field with @binw×@binh if you want sum.
 *
 * By giving non-zero @trimlowest and @trimhighest you can change the plain
 * average to a trimmed one (even turning it to median in the extreme case).
 * It must always hold that @trimlowest + @trimhighest is smaller than
 * @binw×@binh.
 *
 * Returns: A newly created data field.
 *
                          ...
                          gint trimlowest, gint trimhighest)
{
    ...
    binsize = binw*binh;
    g_return_val_if_fail(trimlowest >= 0
                         && trimlowest < binsize, NULL);
    g_return_val_if_fail(trimhighest >= 0
                         && trimhighest < binsize - trimlowest, NULL);
    if (binsize == 1)
        return gwy_data_field_duplicate(data_field);
    ...
    result = gwy_data_field_new(newxres, newyres,
                                newxres*xreal*binw/xres,
                                newyres*yreal*binh/yres,
                                FALSE);
    ...
    for (i = 0; i < newyres; i++) {
        for (j = 0; j < newxres; j++) {
            ...
            r[i*newxres + j] = gwy_math_trimmed_mean(binsize, buf,
                                                     trimlowest, trimhighest);
        }
    }
    g_free(buf);
...
_gwy_data_field_check_mask(GwyDataField *data_field,
                           ...
                           GwyMaskingType *masking)
{
    /* NULL @mask is a direct caller error.  We allow NULL @masking for old
     * functions that do not have mode so masking is implicitly done in INCLUDE
     * mode. */
    g_assert(mask);
    if (!*mask) {
        if (masking)
    ...
    if (masking) {
        ...
        g_return_val_if_fail(*masking == GWY_MASK_INCLUDE
                             || *masking == GWY_MASK_EXCLUDE, FALSE);
    }
    g_return_val_if_fail(GWY_IS_DATA_FIELD(data_field), FALSE);
