/*
 *  $Id: filters-minmax.c 23296 2021-03-18 00:17:54Z yeti-dn $
...
/* K-th rank filter tree.  Number of levels with special storage; the rest
 * is in ucount[][]. */
enum { MEDIAN_RADIX_TREE_BASE_LEVELS = 8 };

/* This needs somewhat less than 2n bits, i.e. n/4 bytes.  We actually never
 * use the last element in each quadruple – but having it enables making
 * add/remove branch-free, using just shifts and increments/decrements. */
typedef struct {
    guint n;
    guint levels;
 ...
 * gwy_data_field_area_filter_min_max() to perform filtering with different,
                                    ...
                                    gint width, gint height)
{
    ...
    for (i = 0; i <= extend_down; i++) {
        ...
        if (row + i < yres) {
            ...
            min_max_precomputed_row_copy(prows[ii], prows[ii-1],
                                         req, rowbuflen);
        }
    }
                                        ...
                                        gint width, gint height)
{
    ...
    if (filtertype == GWY_MIN_MAX_FILTER_MINIMUM
        ...
        gwy_data_field_area_rle_analyse(kernel, width, &mmp);
        gwy_data_field_area_min_max_execute(data_field, outbuf2, &mmp, FALSE,
                                            col, row, width, height);
...
#pragma omp parallel if (gwy_threads_are_enabled()) default(none) \
...
#endif
        {
            ...
            gwy_data_field_area_filter_min_max_real(data_field, redkernel, b,
                                                    filtertype,
                                                    col, ifrom,
                                                    width, ito - ifrom);
...
#pragma omp parallel if (gwy_threads_are_enabled()) default(none) \
...
#endif
        {
            ...
            gwy_data_field_area_filter_min_max_real(data_field, kernel, b,
                                                    filtertype1,
                                                    col, ifrom,
                                                    width, ito - ifrom);
        }
        gwy_data_field_area_copy(buf, data_field,
                                 0, 0, width, height, col, row);
 ...
 *
 * This method uses a simple square kernel.  Use the general function
 * gwy_data_field_area_filter_kth_rank() to perform filtering with different,
 * for instance circular, kernel.
 **/
void
...
/* The compiler should be clever enough to avoid repeated comparison of levels
 * with the constants.  GCC seems to be. */
static void
...
/* This can be relatively complicated compared to add/remove because we run
 * add/remove twenty or hundred times per one k-th rank invocation. */
static guint
                                ...
                                guint xres)
{
    ...
    for (i = 0; i < kyres; i++) {
        ...
        for (j = 0; j < kxres; j++, k++, kk++) {
            ...
            /* Use xres from the image, not the kernel.  This means the indices
             * are directly usable with the image. */
                             ...
                             GwyDataField *kernel)
{
    ...
    if (extcol >= 0 && extrow >= 0 && extxend <= xres && extyend <= yres) {
        return gwy_data_field_area_extract(dfield,
                                           extcol, extrow, extw, exth);
    }
    ...
    /* When we both crop and extend, crop first, then border-extend the
     * parts we need to extend. */
    tmp = gwy_data_field_area_extract(dfield,
                                      MAX(extcol, 0), MAX(extrow, 0),
                                      MIN(extxend, xres) - MAX(extcol, 0),
                                      MIN(extyend, yres) - MAX(extrow, 0));
    dfield = gwy_data_field_extend(tmp,
                                   MAX(-extcol, 0), MAX(extxend-xres, 0),
                                  ...
                                  gboolean *ok)
{
    ...
    /* Initialise the tree with ranks corresponding to kernel in top left
     * corner. */
    ...
    /* Go vertically in the inner loop because then we tend to access ranks[]
     * in contiguous blocks.  Recycle the revindex[] array as filtered[]. */
    filtered = revindex;
    workdone = 0;
    j = 0;
    while (!cancelled) {
        ...
        workdone += kn*h;
        /* Check for cancellation in the master thread because set_fraction()
         * can make GTK+ calls.
 ...
 *
 * Pass half the number of non-zero values in @kernel as @k for a median
 * filter.
 *
 ...
 *
 * Returns: %TRUE if the operation was not aborted via @set_fraction
 *          returning %FALSE; %FALSE if it was aborted.
 *
 * Since: 2.51
 **/
gboolean
                                    ...
                                    GwySetFractionFunc set_fraction)
{
    ...
    if (k == 0) {
        gwy_data_field_area_filter_min_max(data_field, kernel,
    ...
    redkernel = gwy_data_field_duplicate(kernel);
    /* FIXME: It seems there is no way to counteract the autocrop so off-centre
     * kernel do not work! */
    gwy_data_field_grains_autocrop(redkernel, TRUE, NULL, NULL, NULL, NULL);
    if (n <= 25) {
        /* The filter should be fast for tiny kernels. */
        area_rank_filter_kernel_direct(data_field, col, row, width, height,
                                       redkernel, k, n);
        g_object_unref(redkernel);
        return TRUE;
    }
...
#pragma omp parallel if (gwy_threads_are_enabled()) default(none) \
...
#endif
    {
        ...
        /* XXX: We do all the kernel analysis repeatedly. */
        area_rank_filter_kernel_radixtree(data_field,
                                          col, ifrom, width, ito - ifrom,
                                          redkernel, k, set_fraction, &ok);
    }
    g_object_unref(redkernel);
...
/* Add a value, updating integer value interval boundaries f1 and f2 and the
 * running sum. */
static void
...
/* Remove a value, updating integer value interval boundaries f1 and f2 and the
 * running sum. */
static void
...
/* Assume strictly k1 < n-1 - k2.  This allows assuming strictly f1 < f2 all
 * the time. Value within the rank interval [k1, n-1 - k2] (inclusive) are used
 * for averaging.
 *
 * This filter algorithm is written for the case of small k1 and k2, i.e.
 * trimming of the tails.  If the filter output is calculated just from a small
 * interval of ranks, it would be more efficient to calculate the average value
                                   ...
                                   gboolean *ok)
{
     ...
     *
     * We must not cause violation of f1 < f2, which would occur if we removed
     * to many values.  So instead of doiing add/remove in block, we always add
     * one, remove one, add one, remove one, etc.  And keep tmstate consistent
    ...
    while (!cancelled) {
        if (j % 2 == 0) {
            ...
            while (i < h-1) {
                rr = ranks + i*extxres + j;
                for (n = 0; n < kbound.ndown; n++) {
                    median_radix_tree_add_tmean(&mrtree, &tmstate, e,
                                                rr[kbound.down[n] + extxres]);
                    median_radix_tree_remove_tmean(&mrtree, &tmstate, e,
                                                   rr[kbound.up[n]]);
                }
                i++;
    ...
    if (!cancelled) {
        ...
        gwy_data_field_area_multiply(dfield, col, row, w, h,
                                     1.0/(kn - nlowest - nhighest));
    }
...
#endif
    {
        ...
        for (i = ifrom; i < ito; i++) {
            for (j = 0; j < w; j++) {
                ...
                for (n = 0; n < ksize; n++)
                    buf[n] = e[kindex[n]];
                d[i*xres + j] = gwy_math_trimmed_mean(ksize, buf,
                                                      nlowest, nhighest);
            }
        }
 ...
 *
 * At least one value must remain after the trimming, i.e. @nlowest + @nhighest
 * must be smaller than the number of non-zero values in @kernel.  Usually one
 * passes the same number as both @nlowest and @nhighest, but it is not
 * a requirement.
 *
 * The kernel field is a mask that defines the shape of the kernel.  You can
 * use gwy_data_field_elliptic_area_fill() to create a true circular (or
                                        ...
                                        GwySetFractionFunc set_fraction)
{
    ...
    if (nlowest + nhighest + 1 == n) {
        return gwy_data_field_area_filter_kth_rank(data_field, kernel,
                                                   col, row, width, height,
                                                   nlowest, set_fraction);
    }
    ...
    if (!nhighest && !nlowest) {
        ...
        gwy_data_field_area_ext_convolve(data_field, col, row, width, height,
                                         data_field, redkernel,
    ...
    if (n <= 40) {
        /* The filter should be fast for tiny kernels. */
        area_tmean_filter_kernel_direct(data_field, col, row, width, height,
                                        redkernel, nlowest, nhighest, n);
        g_object_unref(redkernel);
        return TRUE;
    }
...
#pragma omp parallel if (gwy_threads_are_enabled()) default(none) \
...
#endif
    {
        ...
        /* XXX: We do all the kernel analysis repeatedly. */
        area_tmean_filter_kernel_radixtree(data_field,
                                           col, ifrom, width, ito - ifrom,
                                           redkernel, nlowest, nhighest,
                                           set_fraction, &ok);
    }
