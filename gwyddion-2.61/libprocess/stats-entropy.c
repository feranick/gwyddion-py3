/*
 *  $Id: stats-entropy.c 21661 2018-11-21 13:28:44Z yeti-dn $
                            ...
                            gdouble *S)
{
     ...
     * Trees covering overlapping (or the same) intervals are lots of work to
     * merge.  Processing them separately is possible, but then we hit
     * different number of add_estimated_unsplit_node_entropy() depending on
     * the number of threads.
     *
     * So one option is add a gwy_math_kth_ranks() preprocessing.  It finds the
     * values that split the data to equal chunks.  And not only that, it also
     * physically splits them, so each chunk becomes a continuous block in the
     * array.  Neat!
     *
     * So each thread can construct its own, independent tree.  However, the
     * result still depends on the number of threads because the bin boundaries
     * are not the same as for serial processing.
     *
     * For deterministic results each thread must essentially construct
     * exactly one branch of the tree.  We must assign an interval of the
     * input of length (max-min)/2^n for some n and also at position
     * corresponding to a branch.  Then we just merge the branches to form
     * a common root.
     *
     * Splitting the work reasonably requires a fair amount of initial
     * analysis.   The result could be for instance that if we have 6 threads
     * they should process 1/4, 1/4, 1/16, 1/16, 1/8 and 1/4 of the entire
     * interval of values.
     *
     * For a key commonly used function it may be woth implementing this.
     * Here, it is hardly a priority. */
