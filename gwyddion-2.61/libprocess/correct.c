/*
 *  $Id: correct.c 23824 2021-06-10 13:19:16Z yeti-dn $
                                ...
                                gpointer user_data)
{
    ...
    if (exterior == GWY_EXTERIOR_LAPLACE) {
        g_warning("Laplace exterior cannot be used with distortions.  "
...
#pragma omp parallel for if(gwy_threads_are_enabled()) default(none) \
        ...
        for (newi = 0; newi < newyres; newi++) {
            ...
            for (newj = 0; newj < newxres; newj++, k++)
                invtrans(newj + 0.5, newi + 0.5,
                         &my_coords[k].x, &my_coords[k].y, user_data);
        }
        coords = my_coords;
    }
 ...
 *
 * Data line @shifts must have resolution corresponding to the number of
 * @data_field rows.  Its values are subtracted from individual field rows.
 *
 * Since: 2.52
 **/
void
 ...
 * @data_field: A data field.
 * @mask: Mask of values to take values into account/exclude, or %NULL for full
 *        @data_field.
 * @masking: Masking mode to use.  See the introduction for description of
 *           masking modes.
 * @trimfrac: Fraction of lowest values and highest values to discard when
 *            trimming.
 * @mincount: Minimum number of values in a row necessary for per-row
 ...
 *            number to use an automatic minimum count.
 *
 * Finds row shifts to misaligned row correction using trimmed row means.
 *
 * For zero @trimfrac the function calculates row means.  For @trimfrac of 1/2
 * or larger it calculates row medians.  Values between correspond to trimmed
 * means.
 *
 * Returns: A newly created data line containing the row shifts, for instance
 *          row means, medians or trimmed means.
 *
 * Since: 2.52
 **/
GwyDataLine*
                                            ...
                                            gint mincount)
{
    ...
    total_median = gwy_data_field_area_get_median_mask(data_field,
                                                       mask, masking,
                                                       0, 0, xres, yres);
 ...
 *            number to use an automatic minimum count.
 *
 * Finds row shifts to misaligned row correction using trimmed means of row
 * differences.
 *
 * For zero @trimfrac the function calculates row means.  For @trimfrac of 1/2
 * or larger it calculates row medians.  Values between correspond to trimmed
